-- DIARNEC — Dashboard semanal (solo staff: admin / encargado).
--
--  · dash_periodo(desde, hasta): todo lo del período en un solo llamado — indicadores, ventas por día y
--    rankings (productos ganadores, vendedores, productos más devueltos, marcas, categorías, clientes).
--    El Dashboard lo llama una vez por semana elegida y otra por la semana con la que se compara;
--    el asistente también lo usa para cualquier período (hoy, mes, últimos N días...).
--  · dash_tendencia(n): las últimas n semanas (lunes a domingo, hora de Argentina) para el selector
--    y el gráfico de evolución.
--
-- Definiciones (todas sin lo anulado):
--   ventas_neto   = lo que se le factura a quien retira (total_neto: precio final menos comisión)
--   devoluciones / bonificaciones = créditos que se descuentan de esa facturación
--   cobros        = pagos de vendedores + pagos de clientes
--   egresos       = gastos + pagos a proveedores (lo realmente pagado)
--   costo_estimado = unidades vendidas x costo ACTUAL del producto (el costo histórico no se guarda)
...
revoke all on function dash_periodo(date, date), dash_tendencia(int) from public, anon;
grant execute on function dash_periodo(date, date), dash_tendencia(int) to authenticated;
